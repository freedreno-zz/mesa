 ...
 *         IMHO, the proper setup is what fixup_memory_mappings() does on
...
/*
 * Ok, this is a bit complicated. Basically, "normal" 800x600 8bpp mode
 * with a 800 bytes pitch isn't suitable for the accel engine. We need
 * to either use a different pitch or tweak the accel routines.
 * 
 * My first implementation made sure the pitch was set so that the
 * accel code would always work. However, some (quite rare though)
 * legacy apps expect a hard coded pitch of 800 bytes when using
 * this mode (ugh) and Daniel Mantione provided me with a version of
 * the accel code that can deal with that.
 * 
 * The #define below will be set to 1 to use that alternate solution,
 * or to 0 to use the simple alignement fix
 * 
 * Note that other modes may be affected as well and the accel engine
 * tweak works only for 800x600. For such modes, we currently disable
 * acceleration. Change the alignement fix if you want to keep accel
 * working for these.
 * 
 * --BenH.
 */
#define USE_SPECIAL_ACCEL	1
...
static struct pci_device_id radeonfb_pci_table[] __devinitdata = {
	...
	{ PCI_VENDOR_ID_ATI, PCI_DEVICE_ID_ATI_RADEON_LR, PCI_ANY_ID, PCI_ANY_ID, 0, 0, RADEON_LR},
...
#if USE_SPECIAL_ACCEL
static struct display_switch fbcon_radeon8_special;
#endif
#endif
...
MODULE_PARM_DESC(noaccel, "Disable (1) or enable (0) the usage of the 2d-accelerator");
...
use_defaults:
		 ...
		 * better mecanism to get them. I also didn't add all cards.
		 */
		switch (rinfo->chipset) {
			...
			case PCI_DEVICE_ID_ATI_RADEON_AE:
			case PCI_DEVICE_ID_ATI_RADEON_AF:
	...
	if (!rinfo->got_dfpinfo) {
		...
		switch (rinfo->panel_yres) {
			...
			case 600:
				...
				break;
			case 768:
#if defined(__powerpc__) /* Kill that horror ! --BenH. */
				if (rinfo->dviDisp_type == MT_LCD)
...
#if USE_SPECIAL_ACCEL
        rinfo->special_accel = 0;
        temp = rinfo->xres_virtual * (rinfo->bpp / 8);
        if ((rinfo->bpp == 8) && ((temp & 0x3f) == 0x20)) {
                rinfo->special_accel = 1;
                temp = temp << 1;
        } else if (temp & 0x3f) {
                printk(KERN_INFO "radeonfb: 2d-acceleration disabled; line length %d unsuitable\n", temp);
                return -EINVAL;
        };
	rinfo->pitch = temp >> 6;
#endif	/* USE_SPECIAL_ACCEL */
...

{
        ...
        switch (disp->var.bits_per_pixel) {
                ...
                case 8:
                        ...
                        if (accel)
                                if (rinfo->special_accel)
                                        disp->dispsw = &fbcon_radeon8_special;
                                else
                                        disp->dispsw = &fbcon_radeon8;
                        else
	...
	if (!noaccel) {
        ...
        if (chgvar) {     
	        radeon_set_dispsw(rinfo, disp);
                ...
                if (noaccel || (disp->var.accel_flags & FB_ACCELF_TEXT) == 0)
                        disp->scrollmode = SCROLL_YREDRAW;
                else
                        disp->scrollmode = 0;
                               ...
                               struct radeon_regs *save)
{
	...
	save->mc_fb_location = INREG(MC_FB_LOCATION);
	save->display_base_addr = INREG(DISPLAY_BASE_ADDR);
}
...
static void fbcon_radeon_bmove_r(struct display *p, int srcy, int srcx,
			       ...
			       int dsty, int dstx, int height, int width)
{
	...
	height *= fontheight(p);
        ...
        fbcon_radeon_bmove_r(p,srcy,srcx,dsty,dstx,height,width);
};
...
/*
 * When accelerating a video mode that has a line length that is not a
 * multiple of 64 bytes we have a problem: the 2d-engine does not support
 * it. This happens for example in 800x600x8, where the line length is 800
 * bytes. To accelerate this we program the double line length into
 * the engine. In our drawing operations we then compensate for this;
 * we do the operation twice, once for the even lines and once for
 * the odd lines
 */

static void fbcon_radeon8_bmove_spc(struct display *p, int srcy, int srcx,
			            int dsty, int dstx, int height, int width) {
	struct radeonfb_info *rinfo = (struct radeonfb_info *)(p->fb_info);
        int nsrcy,ndsty;

	nsrcy = srcy * fontheight(p);
	ndsty = dsty * fontheight(p);

        if ((nsrcy - ndsty) & 1) {
                /* Too much trouble to accelerate this, would need a temp buffer.*/
	        ...
	        height *= fontheight(p);
                if (srcy & 1) {
                        fbcon_radeon_bmove_r(p,(srcy >> 1) + 1,srcx,(dsty >> 1) + 1,dstx,(height >> 1),width);
                        fbcon_radeon_bmove_r(p,(srcy >> 1),rinfo->xres_virtual + srcx,(dsty >> 1),rinfo->xres_virtual + dstx,(height >> 1),width);
                } else {
                        fbcon_radeon_bmove_r(p,(srcy >> 1),srcx,(dsty >> 1),dstx,(height >> 1) + (height & 1),width);
                        fbcon_radeon_bmove_r(p,(srcy >> 1),rinfo->xres_virtual + srcx,(dsty >> 1),rinfo->xres_virtual + dstx,(height >> 1),width);
                };
        };
};

static void fbcon_radeon8_clear_spc(struct vc_data *conp, struct display *p,
			            int srcy, int srcx, int height, int width)
{
	struct radeonfb_info *rinfo = (struct radeonfb_info *)(p->fb_info);
	u32 clr;
	...
	height *= fontheight(p);
        ...
        if (srcy & 1) {
        	radeon_rectfill(rinfo, (srcy >> 1) + 1, srcx, (height >> 1), width, clr);
        	radeon_rectfill(rinfo, (srcy >> 1), rinfo->xres_virtual + srcx, (height >> 1) + (height & 1), width, clr);
        } else {
        	radeon_rectfill(rinfo, (srcy >> 1), srcx, (height >> 1) + (height & 1), width, clr);
        	radeon_rectfill(rinfo, (srcy >> 1), rinfo->xres_virtual + srcx, (height >> 1), width, clr);
        }
}

static struct display_switch fbcon_radeon8_special = {
	setup:			fbcon_cfb8_setup,
	bmove:			fbcon_radeon8_bmove_spc,
	clear:			fbcon_radeon8_clear_spc,
	putc:			fbcon_cfb8_putc,
	putcs:			fbcon_cfb8_putcs,
	revc:			fbcon_cfb8_revc,
 	clear_margins:		fbcon_cfb8_clear_margins,
